#!/usr/bin/env bash
# Build Hizashi Sách 06 (Đàm phán·Đề xuất / 商談・交渉) → EPUB
...
CSS="$SCRIPT_DIR/epub_negotiation.css"
...
if [ -f "$ANALYZE" ]; then
    python3 "$ANALYZE" "$NOI_DUNG" --gen-css negotiation \
...
if command -v pandoc &> /dev/null; then
    ...
    EPUB_OUT="$OUTPUT/Hizashi_negotiation_v${VERSION}.epub"
