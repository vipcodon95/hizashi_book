...
VERSION="1.1"
TITLE="Hizashi — Thuyết trình / プレゼンテーション v${VERSION}"
...
if command -v pandoc &> /dev/null; then
    echo "Building docx..."
    pandoc "$COMBINED" -o "$OUTPUT/Hizashi_presentation_v${VERSION}.docx" \
        --toc --toc-depth=2 \
    ...
    echo "Building html..."
    pandoc "$COMBINED" -o "$OUTPUT/Hizashi_presentation_v${VERSION}.html" \
        --toc --toc-depth=2 --standalone \
        --metadata title="$TITLE" \
        2>/dev/null || echo "  (warn) html có warning"

    echo "Building epub..."
    pandoc "$COMBINED" -o "$OUTPUT/Hizashi_presentation_v${VERSION}.epub" \
        --toc --toc-depth=2 \
        --metadata title="$TITLE" \
        --metadata author="Hizashi Teams" \
        --metadata lang=vi \
    ...
    echo ""
    echo "✓ Done. Output:"
    ls -lh "$OUTPUT"/Hizashi_presentation_v${VERSION}.* 2>/dev/null || true
else
    echo ""
