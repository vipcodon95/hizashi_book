#!/usr/bin/env bash
# Build Hizashi Sách 04 (Báo·Liên·Thảo luận / 報連相) → EPUB
...
CSS="$SCRIPT_DIR/epub_horenso.css"
...
if [ -f "$ANALYZE" ]; then
    python3 "$ANALYZE" "$NOI_DUNG" --gen-css horenso \
        --out-css "$GEN_CSS" --out-lua "$GEN_LUA" || echo "  warn: analyze_tables"
    # Sách 04 RIÊNG: 1 header bảng chứa '\' (Ảnh hưởng \ Rollback) → backslash trần
    # làm hỏng cú pháp Lua → GEN={} → mọi bảng auto-gen mất class. Escape \ → \\.
    if [ -f "$GEN_LUA" ]; then
        python3 - "$GEN_LUA" <<'PYESC'
import sys, re
p = sys.argv[1]
t = open(p, encoding="utf-8").read()
# escape backslash KHÔNG đứng trước escape hợp lệ (\\, \", \n, \t) bên trong các chuỗi header="..."
def fix(m):
    return m.group(0).replace("\\", "\\\\").replace('\\\\"', '\\"')
t2 = re.sub(r'header="[^"]*"', fix, t)
if t2 != t:
    open(p, "w", encoding="utf-8").write(t2)
    print("  ✓ escaped backslash trong _tables_gen.lua (sách 04)")
PYESC
    fi
fi
...
if command -v pandoc &> /dev/null; then
    ...
    EPUB_OUT="$OUTPUT/Hizashi_horenso_v${VERSION}.epub"
