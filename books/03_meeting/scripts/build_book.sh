#!/usr/bin/env bash
# Build Hizashi Sách 03 (Họp / 会議) → EPUB
...
CSS="$SCRIPT_DIR/epub_meeting.css"
...
if [ -f "$ANALYZE" ]; then
    python3 "$ANALYZE" "$NOI_DUNG" --gen-css meeting \
...
if command -v pandoc &> /dev/null; then
    ...
    EPUB_OUT="$OUTPUT/Hizashi_meeting_v${VERSION}.epub"
