#!/usr/bin/env bash
# Build Hizashi Sách 07 (Tiếp khách·Thăm·Danh thiếp / 来客・訪問・名刺交換) → EPUB
...
CSS="$SCRIPT_DIR/epub_visit_card.css"
...
if [ -f "$ANALYZE" ]; then
    python3 "$ANALYZE" "$NOI_DUNG" --gen-css visit_card \
...
if command -v pandoc &> /dev/null; then
    ...
    EPUB_OUT="$OUTPUT/Hizashi_visit_card_v${VERSION}.epub"
