#!/usr/bin/env bash
# Build Hizashi Sách 11 (Thực tập sinh ngành thực phẩm / 技能実習生) → EPUB
# Ghép 12 LyThuyet.md (giáo trình theo tháng) → 1 EPUB. CSS riêng, bìa, front/back matter, furigana.
set -e
...
# 12 chương LyThuyet theo thứ tự t01..t12 (mỗi file có h1 riêng → KHÔNG chèn heading, KHÔNG '---').
for chapter_dir in $(ls -1 "$BOOK_ROOT/" | grep "^t[0-9]" | sort); do
    f=$(ls "$BOOK_ROOT/$chapter_dir/"*LyThuyet*.md 2>/dev/null | head -1)
    if [ -f "$f" ]; then
        echo "" >> "$COMBINED"
        cat "$f" >> "$COMBINED"
        echo "" >> "$COMBINED"
...
if [ -f "$ANALYZE" ]; then
    # quét chỉ các thư mục chương (tránh _pipeline)
    python3 "$ANALYZE" "$BOOK_ROOT" --gen-css jisshusei \
...
if t2 != t:
    ...
    print("  ✓ escaped backslash trong _tables_gen.lua (sách 11)")
PYESC
    fi
fi
...
if command -v pandoc &> /dev/null; then
    ...
    EPUB_OUT="$OUTPUT/Hizashi_jisshusei_v${VERSION}.epub"
